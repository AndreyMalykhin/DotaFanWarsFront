export default {
    'common.genericError': 'Упс, что-то пошло не так.',
    'common.select': 'Выбрать',
    'common.save': 'Сохранить',
    'common.cancel': 'Отмена',
    'common.back': 'Назад',
    'common.ok': 'OK',
    'locale.en': 'EN',
    'locale.ru': 'РУ',
    'mainMenu.profile': 'Профиль',
    'mainMenu.userLeaderboard': 'Рейтинг игроков',
    'mainMenu.teamLeaderboard': 'Рейтинг команд',
    'mainMenu.matchSchedule': 'Матчи',
    'matchSchedule.empty': 'Пока матчей нет :(',
    'matchScheduleItem.live': 'В процессе',
    'matchScheduleItem.join': 'Войти',
    'match.noFreeSlotsError': 'Нет свободных мест',
    'match.leaverError': 'Вы не можете зайти в новую игру, так как преждевременно покинули прошлую. Пожалуйста подождите несколько минут',
    'match.youDead': 'Вы мертвы',
    'match.goalTutorial': 'Ваша задача - убить как можно больше игроков вражеской команды до конца реального матча Доты. Команда, которая совершит больше убийств, будет победителем и все ее игроки получат очки рейтинга, а игроки проигравшей команды эти очки потеряют.',
    'loginDlg.title': 'Вход',
    'loginDlg.google': 'Google',
    'loginDlg.facebook': 'Facebook',
    'userForm.rating': 'Рейтинг',
    'userForm.nickname': 'Ник',
    'userForm.country': 'Страна',
    'userForm.changePhoto': 'Изменить',
    'roomPicker.title': 'Выбор комнаты',
    'targetInfo.rating': 'Рейтинг',
    'item.buy': 'Купить',
    'matchResultDlg.title': 'Результат матча',
    'matchResultDlg.winner': 'Победа {name}!',
    'matchResultDlg.draw': 'Ничья',
    'matchResultDlg.rating': 'Рейтинг',
    'chatInput.placeholder': 'Сообщение',
    'chatInput.send': 'Отправить',
    'seats.tutorial': 'Это места. Здесь вы можете увидеть других игроков и взаимодействовать с ними. Ваши враги подсвечены красным цветом. Пожалуйста займите свободное место нажав на него!',
    'items.tutorial': 'Это вещи. Вы можете использовать их нажав на них. Некоторые вещи требуют цель для их использования, в этом случае вещь будет подсвечена при нажатии на нее.',
    'userLeaderboard.rank': 'Ранг',
    'userLeaderboard.player': 'Игрок',
    'userLeaderboard.rating': 'Рейтинг',
    'characterStats.tutorial': 'Это ваши атрибуты (здоровье, золото). Вы получаете золото каждый раз при совершении убийства вашей командой в реальном матче Доты.',
    'scoreboard.tutorial': 'Это табло. Здесь вы можете увидеть счет реального матча Доты.'
};
